function unstable_sol=Sepparatrix_Keratinocyte_Differentiation_ODE_Model_SS_Int(PKC,NFkB, IL4, v_Np63,va_Np63, d_Stat3, k_Np63, n_H, v_Stat3, d_Np63, d_PKC, a_EDC, i_EDC, d_EDC, aux1, aux2)
...
%% declare the model variables as symbolic variables
    ...
    %% now we will loop over all the solutions and select only those that are positive, real and UNstable
                     ...
                     if (sum(double(eigenvals)<0))==4
                      %   disp('solution is stable :)')
                       ...
                       %  disp('solution is unstable :(')
                         ...
                         unstable_sol=[unstable_sol; double( [sol.Stat3_t(sol_num) sol.Np63_t(sol_num) sol.EDC_t(sol_num) sol.aux_t(sol_num)])];
